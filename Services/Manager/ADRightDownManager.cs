...
{
    public class ADRightDownManager : IADManager
    {
        readonly SQLRepository<ADRightDown> _sqlrepository;
        readonly SQLRepository<ADSet> _adsetsqlrepository;
        readonly SQLRepository<Img> _imgsqlrepository;
        public ADRightDownManager(SQLRepository<ADRightDown> sqlrepository)
        {
        ...
        public string UpdateSeq(int id, int seq, string type, string account, string username)
        {
            ...
            {
                if (seq <= 0) { seq = 1; }
                var oldmodel = new List<ADRightDown>();
                oldmodel = _sqlrepository.GetByWhere("ID=@1", new object[] { id }).ToList<ADRightDown>();
                ...
                {
                    IList<ADRightDown> mtseqdata = null;
                    ...
                    IList<ADRightDown> ltseqdata = null;
        ...
        public string Update(ADEditModel model, string account, string username)
        {
            ...
            {
                Link_Href = model.Link_Href,
                AD_Height = model.AD_Height == null ? 265 : model.AD_Height,
                AD_Name = model.AD_Name,
                AD_Width = model.AD_Width == null ? 200 : model.AD_Width,
        ...
        {
            // if (id.IsNullorEmpty == false) { return new ADEditModel(); }
            var model = new ADEditModel();
            var data = _sqlrepository.GetByWhere("ID=@1", new object[] { id });
            if (data.Count() > 0)
            {
                _sqlrepository.Mapping<ADRightDown, ADEditModel>(data.First(), model);
            }
        ...
        #region PagingForWebSite
        public Paging<ADBase> PagingForWebSite(ADSearchModel model)
        {
            var Paging = new Paging<ADBase>();
            var data = new List<ADBase>();
            var whereobj = new List<string>();
            var wherestr = new List<string>();
            var idx = 1;
            ...
            data = _sqlrepository.GetByWhere("Lang_ID=@1 and Enabled=@2  and Stype=@3 Order By Sort", new object[] { model.Lang_ID, true, model.SType }).ToList<ADBase>(); ;
            data = data.Where(v => (v.StDate == null || v.StDate <= DateTime.Now) && (v.EdDate == null || v.EdDate.Value.AddDays(1) >= DateTime.Now)).ToList();
            Paging.total = data.Count(); ;
            Paging.rows = data.Skip(model.Offset).Take(model.Limit).ToList();
