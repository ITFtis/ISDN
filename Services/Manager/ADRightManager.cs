...
{
    public class ADRightManager : IADManager
    {
        readonly SQLRepository<ADRight> _sqlrepository;
        readonly SQLRepository<ADSet> _adsetsqlrepository;
        readonly SQLRepository<Img> _imgsqlrepository;
        public ADRightManager(SQLRepository<ADRight> sqlrepository)
        {
        ...
        public string UpdateSeq(int id, int seq, string type,string account, string username)
        {
            ...
            {
                if (seq <= 0) { seq = 1; }
                var oldmodel = new List<ADRight>();
                oldmodel = _sqlrepository.GetByWhere("ID=@1", new object[] { id }).ToList<ADRight>();
        ...
        public string Update(ADEditModel model, string account, string username)
        {
            ...
            {
                Link_Href = model.Link_Href,
                AD_Height = model.AD_Height==null? 400 : model.AD_Height,
                AD_Name = model.AD_Name ,
                AD_Width = model.AD_Width == null ? 400 : model.AD_Width,
        ...
        public ADEditModel GetModel(string id)
        {
            ...
            if (data.Count() > 0)
            {
                _sqlrepository.Mapping<ADRight, ADEditModel>(data.First(), model);
            }
