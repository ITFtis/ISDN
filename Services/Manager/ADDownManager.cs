...
{
    public class ADDownManager : IADManager
    {
        readonly SQLRepository<ADDown> _sqlrepository;
        readonly SQLRepository<ADSet> _adsetsqlrepository;
        readonly SQLRepository<Img> _imgsqlrepository;
        public ADDownManager(SQLRepository<ADDown> sqlrepository)
        {
        ...
        public string UpdateSeq(int id, int seq, string type,string account, string username)
        {
            ...
            {
                if (seq <= 0) { seq = 1; }
                var oldmodel = new List<ADDown>();
                oldmodel = _sqlrepository.GetByWhere("ID=@1", new object[] { id }).ToList<ADDown>();
        ...
        public string Update(ADEditModel model, string account, string username)
        {
            ...
            {
                Link_Href = model.Link_Href,
                AD_Height = model.AD_Height==null? 80 : model.AD_Height,
                AD_Name = model.AD_Name ,
        ...
        public ADEditModel GetModel(string id)
        {
            ...
            if (data.Count() > 0)
            {
                _sqlrepository.Mapping<ADDown, ADEditModel>(data.First(), model);
            }
