...
{
    [Authorize]
    public class ADMainManager : IADManager
    {
        readonly SQLRepository<ADMain> _sqlrepository;
        readonly SQLRepository<ADSet> _adsetsqlrepository;
        readonly SQLRepository<Img> _imgsqlrepository;
        public ADMainManager(SQLRepository<ADMain> sqlrepository)
        {
        ...
        public string UpdateSeq(int id, int seq, string type,string account, string username)
        {
            try
            {
                ...
                oldmodel = _sqlrepository.GetByWhere("ID=@1", new object[] { id }).ToList<ADMain>();
                 ...
                 UploadVideoFileDesc="",
                  UploadVideoFileName=model.UploadVideoFileName,
                   UploadVideoFilePath=model.UploadVideoFilePath,
                    ADDesc=model.ADDesc==null?"": model.ADDesc
            };
        ...
        public string Update(ADEditModel model, string account, string username)
        {
            ...
            //刪除舊檔案
            if (model.UploadVideoFileName.IsNullorEmpty())
            {
                if (System.IO.File.Exists(olddata.First().UploadVideoFilePath))
                {
                    System.IO.File.Delete(olddata.First().UploadVideoFilePath);
                }
                model.UploadVideoFilePath = "";
                model.UploadVideoFileName = "";
            }
            else
            {
                if (olddata.First().UploadVideoFileName != model.UploadVideoFileName)
                {
                    if (System.IO.File.Exists(olddata.First().UploadVideoFilePath))
                    {
                        System.IO.File.Delete(olddata.First().UploadVideoFilePath);
            ...
            {
                Link_Href = model.Link_Href,
                AD_Height = model.AD_Height==null? 420 : model.AD_Height,
                AD_Name = model.AD_Name ,
                AD_Width = model.AD_Width == null ? 600 : model.AD_Width,
            ...
            if (r > 0)
            {
                try
                {
                    ...
                    return "修改失敗 Message:"+ ex.Message;
                }
        ...
        public ADEditModel GetModel(string id)
        {
            ...
            if (data.Count() > 0)
            {
                _sqlrepository.Mapping<ADMain, ADEditModel>(data.First(), model);
            }
